...
function SavePassword() {
	...
	// if all checks were here error messages could be very decriptve and page
	// responsive, but
	...
	} else if (document.getElementById("EditUserPassword_newpas1").value != document
			.getElementById("EditUserPassword_newpas2").value) {
	...
	if (valid) {
		...
		if (document.getElementById("EditUserPassword_username") != null) {
			...
			cancel = confirm("Are you sure you want to change " + userName
					+ "'s password?") == false;
		} else
...
function EditCustomer(add, hNo, cNo, name, notes, status) {
	...
	if (add) {
		...
		document.getElementById("EditCustomer_cancelbutton").onclick = function() {
			window.location = './';
		};
	} else {
		document.getElementById("EditCustomerEntryLegend").innerHTML = "Edit Customer ("
				+ name + ")";
...
function SaveCustomer() {
	...
	if (document.getElementById("EditCustomer_hno").value.length < 6
			|| isNaN(document.getElementById("EditCustomer_hno").value)) {
		errorMsg = "Invalid H#";
		FocusAndSelectAll("EditCustomer_hno");
	} else if (document.getElementById("EditCustomer_cno").value.length < 6
			|| isNaN(document.getElementById("EditCustomer_cno").value)) {
		errorMsg = "Invalid C#";
...
function EditBadge(add, badgeID, hNo, name, badgeNo, status, issue) {
	...
	if (add) {
		...
		document.getElementById("EditBadgeEntryLegend").innerHTML = "Edit Badge ("
				+ name + ")";
...
function SaveBadge() {
	...
	if (document.getElementById("EditBadge_hno").value.length < 6
			|| isNaN(document.getElementById("EditBadge_hno").value)) {
		errorMsg = "Invalid H#";
	...
	} else if (document.getElementById("EditBadge_badgeno").value.length <= 0
			|| isNaN(document.getElementById("EditBadge_badgeno").value)) {
...
function EditDevicePort(event, add, permission, devicePortID, deviceID,
		deviceName, portName, pic, port, type, status, speed, mac, note) {
	...
	if (add) {
		...
		document.getElementById("EditDevicePortEntryLegend").innerHTML = "Edit Port ("
				+ deviceName + " " + portName + ")";
...
function DeleteDevicePort() {
	var confirmed = false;
	var confim = prompt(
			"Are you sure you want to delete this Port. Have you already dealt with all outstanding port connections and VLAN links? Enter 'YES' to confirm.",
			"");
...
function SaveDevicePort() {
	...
	if (document.getElementById("EditDevicePort_deviceid").value.length < 1
			|| isNaN(document.getElementById("EditDevicePort_deviceid").value)) {
		errorMsg = "Invalid Device ID";
	} else if (document.getElementById("EditDevicePort_deviceportid").value.length < 1
			|| isNaN(document.getElementById("EditDevicePort_deviceportid").value)) {
		errorMsg = "Invalid Device Port ID";
	} else if (document.getElementById("EditDevicePort_pic").value.length < 1
			|| document.getElementById("EditDevicePort_pic").value.length > 2
			|| isNaN(document.getElementById("EditDevicePort_pic").value)) {
		errorMsg = "Invalid Pic";
		FocusAndSelectAll("EditDevicePort_pic");
	} else if (document.getElementById("EditDevicePort_portno").value.length < 1
			|| document.getElementById("EditDevicePort_portno").value.length > 2
			|| isNaN(document.getElementById("EditDevicePort_portno").value)) {
...
function EditDevice(add, deviceID, hNo, name, type, size, locationID, unit,
		status, notes, model, member, asset, serial) {
	HideAllEditForms();
	// populate fields
	...
	if (add) {
		...
		document.getElementById("EditDeviceEntryLegend").innerHTML = "Edit Device ("
				+ name + ")";
	...
	// these are not post editabled fields - need to be changed by admin -
	// should never really be changed
	...
	SelectItemByValue(document.getElementById("EditDevice_location"),
			locationID);
...
function EditDeviceTypeChanged(updateValues) {
	...
	if (type == "C") {
		...
		if (updateValues) {
			SelectItemByValue(document.getElementById("EditDevice_model"),
					'Unknown');
		}
	} else {
		colo = false;
	...
	if (colo) {
		...
		// set to propper default values - though these probably wont get
		// checked at all
		if (updateValues) {
...
function SaveDevice() {
	...
	/*
	 * if(document.getElementById("EditDevice_asset").value.length < 7 ||
	 * isNaN(document.getElementById("EditDevice_asset").value)){ errorMsg =
	 * "Invalid asset"; FocusAndSelectAll("EditDevice_asset"); }
	 */
	// TODO check for existing... propper member (or other values) here in AJAX
	// - prevent re-entering complex device info
	// TODO popup confirm.. dialog
	// document.getElementById("EditDevice_member").value;
	// hNo, name, notes, unit, size -
	if (document.getElementById("EditDevice_hno").value.length < 6
			|| isNaN(document.getElementById("EditDevice_hno").value)) {
		errorMsg = "Invalid H#";
	...
	} else if (!document.getElementById("EditDevice_unit").disabled
			&& (document.getElementById("EditDevice_unit").value.length < 1 || isNaN(document
					.getElementById("EditDevice_unit").value))) {
		errorMsg = "Invalid Unit. Must be a number"; // null or NAN
		FocusAndSelectAll("EditDevice_unit");
	} else if (!document.getElementById("EditDevice_unit").disabled
			&& (document.getElementById("EditDevice_unit").value > 99 || document
					.getElementById("EditDevice_unit").value < 0)) {
		errorMsg = "Invalid Unit. Must be between 0 and 99";// not between 0 and
		// 99
		FocusAndSelectAll("EditDevice_unit");
	} else if (!colo
			&& document.getElementById("EditDevice_model").value == "Unknown") {
	...
	if (valid) {
		...
		if (add && valid) {
			valid = false;
			var confim = prompt(
					"Are you sure you want to create this device and all of its ports. Enter 'YES' to confirm.",
					"");
			if (confim == "YES")
				valid = true;
			else
		...
		if (valid) {
			// re-enable fields for send - these should be the ones that are
			// disabled dynamicly
...
function EditConnection(add, portConnectionID, childDeviceID, childPortID,
		parentDeviceID, parentPortID, patches, description) {
	HideAllEditForms();
	// populate fields
	if (add) {
		...
		document.getElementById("EditConnectionEntryLegend").innerHTML = "Edit Connection ("
				+ description + ")";
		...
		// select propper ports after AJAX completes - store globaly in window
		// till then
...
function DeviceSelectChanged(isChild, selectedPortID) {
	// TODO should optomise to not call AJAX if this is the same as the last
	// call - duplicate - watch out for same device but dif port (port drop down
	// will be dif on edit)
	var deviceSelect = "";
	var portSelect = "";
	if (isChild) {
...
function SaveConnection() {
	...
	// all data is from selects (or blah notes) data cant be wrong unless
	// hacking
	if (document.getElementById("EditConnection_childdeviceid").value.length < 1
			|| isNaN(document.getElementById("EditConnection_childdeviceid").value)) {
		errorMsg = "Invalid Child Device";
		document.getElementById("EditConnection_childportid").focus();
	} else if (document.getElementById("EditConnection_childportid").value.length < 1
			|| isNaN(document.getElementById("EditConnection_childportid").value)) {
		errorMsg = "Invalid Child Port";
		document.getElementById("EditConnection_childportid").focus();
	} else if (document.getElementById("EditConnection_parentdeviceid").value.length < 1
			|| isNaN(document.getElementById("EditConnection_parentdeviceid").value)) {
		errorMsg = "Invalid Parent Device";
		document.getElementById("EditConnection_parentdeviceid").focus();
	} else if (document.getElementById("EditConnection_parentportid").value.length < 1
			|| isNaN(document.getElementById("EditConnection_parentportid").value)) {
		errorMsg = "Invalid Parent Port";
		document.getElementById("EditConnection_parentdeviceid").focus();
	} else if (document.getElementById("EditConnection_parentportid").value == document
			.getElementById("EditConnection_childportid").value) {
...
function EditCircuit(add, powerID, panel, circuit, volts, amps, status, load) {
	...
	if (add) {
		...
		document.getElementById("EditCircuitEntryLegend").innerHTML = "Edit Circuit ("
				+ panel + " " + circuit + ")";
...
function ValidPowerPanel(input) {
	// could load in array of existing panels and check against that... new
	// panels would have to be added backdoor
...
function SaveCircuit() {
	...
	if (add && !ValidPowerPanel(panelInput.value)) {
		...
		panelInput.focus();
			...
			&& (document.getElementById("EditCircuit_circuit").value.length < 1 || isNaN(document
					.getElementById("EditCircuit_circuit").value))) {
...
function SaveLocation() {
	...
	/*
	 * var add = document.getElementById("EditConnection_action").value ==
	 * "Connection_Add"; document.getElementById("EditConnectionMsg").className =
	 * "hidden";
	 * 
	 * //all data is from selects (or blah notes) data cant be wrong unless
	 * hacking
	 * if(document.getElementById("EditConnection_childdeviceid").value.length <
	 * 1 ||
	 * isNaN(document.getElementById("EditConnection_childdeviceid").value)){
	 * errorMsg = "Invalid Child Device";
	 * document.getElementById("EditConnection_childportid").focus(); } else
	 * if(document.getElementById("EditConnection_childportid").value.length < 1 ||
	 * isNaN(document.getElementById("EditConnection_childportid").value)){
	 * errorMsg = "Invalid Child Port";
	 * document.getElementById("EditConnection_childportid").focus(); } else
	 * if(document.getElementById("EditConnection_parentdeviceid").value.length <
	 * 1 ||
	 * isNaN(document.getElementById("EditConnection_parentdeviceid").value)){
	 * errorMsg = "Invalid Parent Device";
	 * document.getElementById("EditConnection_parentdeviceid").focus(); } else
	 * if(document.getElementById("EditConnection_parentportid").value.length <
	 * 1 ||
	 * isNaN(document.getElementById("EditConnection_parentportid").value)){
	 * errorMsg = "Invalid Parent Port";
	 * document.getElementById("EditConnection_parentdeviceid").focus(); } else
	 * if(document.getElementById("EditConnection_parentportid").value ==
	 * document.getElementById("EditConnection_childportid").value){ errorMsg =
	 * "The child port cannot be the same as the parent port";
	 * document.getElementById("EditConnection_childportid").focus(); } else
	 * valid = true;
	 * 
	 * if(add && valid) { var cancel = confirm("Are you sure you want to create
	 * this Connection?") == false; valid = !cancel; }
	 */
...
function EditSubnet(add, vlanID, portID, vlan, subnet, mask, gateway, first,
		last, note) {
	HideAllEditForms();
	// populate fields
	if (add) {
		...
		document.getElementById("EditSubnetEntryLegend").innerHTML = "Edit Subnet ("
				+ vlan + " " + subnet + ")";
...
function SaveSubnet() {
	...
	if (add && valid) {
		...
		if (newTempVLAN)
			...
			cancel = confirm("Are you sure you want to create this Subnet on VLAN "
					+ VLAN + "?") == false;
		valid = !cancel;
	} else if (valid) {
		var cancel = false;
	...
	// submit
	if (valid) {
		document.getElementById("EditSubnet_vlan").value = document
				.getElementById("EditSubnet_vlan").value.replace("T", "-");
