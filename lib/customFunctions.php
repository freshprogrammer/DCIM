<?php
	...
	class DCIMCustomFunctions
	{
		...
		{	
			...
			{
				//custom layouts
				$query = "SELECT roomid, siteid, name , fullname, custaccess, xpos, ypos, width, depth, orientation, layer
					FROM dcim_room
					WHERE roomid=?";
				...
				$stmt->execute();
				$stmt->store_result();
				$stmt->bind_result($roomID, $siteID, $name , $fullName, $custAccess, $xPos, $yPos, $width, $depth, $orientation, $layer);
