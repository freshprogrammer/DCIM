...
{
	var confirmed;
	confirmed = confirm("Are you sure you want to delete this badge? Have you already updated the badge server?");
...
function SaveCircuit()
{
	...
	else if(parseInt(document.getElementById("EditCircuit_load").value) > parseInt(document.getElementById("EditCircuit_amps").value)){ 
		errorMsg = "Invalid load. Load cannot be greater than the circuit's ampere rating";
		document.getElementById("EditCircuit_load").focus();
	}
	else
		valid = true;
...
function EditLocation(add, locationID, roomID, name, altName, type, units, orientation, x, y, width, depth, note)
{
	HideAllEditForms();
	//populate fields
	if(add)
	...
	//focus
	document.getElementById("EditLocation_roomid").focus();
}
...
function SaveLocation()
{
	...
	if(add && valid)
	{
		var cancel = confirm("Are you sure you want to create this location?") == false;
		valid = !cancel;
	}
...
{
	var confirmed;
	confirmed = confirm("Are you sure you want to delete this location?");
