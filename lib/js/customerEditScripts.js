...
function EditDevice(add, deviceID, roomID, hNo, name,altname, fullName, type, size, locationID, unit, status, notes, model, member, asset, serial)
{
	...
	document.getElementById("EditDevice_model").value = model;
...
{
	var confirmed;
	confirmed = window.confirm("Are you sure you want to delete this device?");
	
	if(confirmed)
	{
		document.getElementById("EditDevice_action").value = "Device_Delete";
		document.getElementById("EditDeviceForm").submit();
	}
}
