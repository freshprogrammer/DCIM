...
function EditDevice(add, deviceID, hNo, name,altname, fullName, type, size, locationID, unit, status, notes, model, member, asset, serial)
{
	HideAllEditForms();
	//populate fields
...
function DeviceSelectChanged(isChild, selectedPortID)
{
	...
	var page = "lib/ajax_devicePortSelect.php";
...
function EditPowerCircuit(add, powerCircuitID, panel, circuit, volts, amps, status, load)
{
	HideAllEditForms();
	//populate fields
	if(add)
	...
	{
		document.getElementById("EditPowerCircuitEntryLegend").innerHTML = "Edit Power Circuit ("+panel+" "+circuit+")";
	...
	document.getElementById("EditPowerCircuit_panel").readOnly = !add;
	...
	document.getElementById("EditPowerCircuit_panel").value = panel;
	...
	//focus
	if(add)
		document.getElementById("EditPowerCircuit_panel").focus();
	else
		document.getElementById("EditPowerCircuit_amps").focus();
}
...
function SavePowerCircuit()
{
	...
	var panelInput = document.getElementById("EditPowerCircuit_panel");
	
	if(add && !ValidPowerPanel(panelInput.value)){ 
		errorMsg = "Invalid Power Panel";
		panelInput.focus();
	}
	else if(add && (document.getElementById("EditPowerCircuit_circuit").value.length < 1 || isNaN(document.getElementById("EditPowerCircuit_circuit").value))){ 
